...
// 트리 역추적 - dfs - 재귀
public class Main_백준_2263_트리의순회_G1_404ms {
	static StringBuilder sb;
	static int[] inorder, post;
	static int[] inorderIdx;
	
	static final void findPre(int postL, int postR, int inL, int inR) {
...
//		int sizeR = inR - rootIdx; // 왼쪽트리 개수가 정해지면 오른쪽트리의 노드개수도 정해짐
		
		findPre(postL, postL + sizeL - 1, inL, rootIdx - 1);
		findPre(postL + sizeL, postR - 1, rootIdx + 1, inR);
	}

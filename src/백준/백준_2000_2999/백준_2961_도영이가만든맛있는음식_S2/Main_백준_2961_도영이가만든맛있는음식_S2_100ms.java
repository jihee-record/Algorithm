...
public class Main_백준_2961_도영이가만든맛있는음식_S2_100ms {
	...
	static void calcul(int idx, int totalSour, int totalBitter) {
		if(idx == N) {
			if(totalBitter != 0) {
		...
		calcul(idx + 1, totalSour * food[idx].sour, totalBitter + food[idx].bitter);
		calcul(idx + 1, totalSour, totalBitter);
	}

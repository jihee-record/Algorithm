...
public class Main_백준_17144_미세먼지안녕_G4 {
	static int R, C, T;
	static int[][] map;
			...
			for(int c=0; c<C; c++) {
				if(map[r][c] > 0) { // 먼지가 있을 때
					...
					for(int d=0; d<4; d++) {
						...
						// 확산ok
						temp[nr][nc] = map[r][c]/5; // 5분의 1만큼 그 방향으로 확산
						remain -= temp[nr][nc]; // 확산 된 만큼 본래 위치에서는 먼지가 빠짐
					}
					temp[r][c] += remain; // 여기서 확산 이후 남은 먼지양 추가
				}
			}
		}
	...
	static void cleanUpper() {
		...
		for(int i=1; i<C; i++) { //오른쪽으로 이동
			c++;
			before = change(before, r, c);
		}
		
		for(int i=1; i<cleaner_up; i--) { //위로 이동
			r--;
			before = change(before, r, c);
		}

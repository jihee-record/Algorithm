...
// 0 : 빈칸, 1~5 : CCTV, 6:벽
public class Main_백준_15683_감시_G3_144ms {
	static char[][] map;
	...
	static void direct(int i, int remain) {
		...
		switch(no) {
		case 1: case1(i, r, c, remain, changed);
			break;
		case 2: case2(i, r, c, remain, changed);
			break;
		case 3: case3(i, r, c, remain, changed);
			break;
		case 4: case4(i, r, c, remain, changed);
			break;
		case 5: case5(i, r, c, remain, changed);
			break;
		}
	}
	...
	static void case1(int i, int r, int c, int remain, ArrayDeque<int[]> changed) {
		for(int d=0; d<4; d++) {
	...
	static void case2(int i, int r, int c, int remain, ArrayDeque<int[]> changed) {
	...
	static void case3(int i, int r, int c, int remain, ArrayDeque<int[]> changed) {
		for(int k=0; k<2; k++) {
	...
	static void case4(int i, int r, int c, int remain, ArrayDeque<int[]> changed) {
		for(int k=0; k<4; k++) {
			int tempRemain = remain;
			for(int d=0; d<4; d++) {
				if(k == d) continue;
	...
	static void case5(int i, int r, int c, int remain, ArrayDeque<int[]> changed) {
		int tempRemain = remain;
		for(int d=0; d<4; d++) {
			tempRemain -= common(r, c, d, changed);
		}
	...
	static int common(int nr, int nc, int d, ArrayDeque<int[]> changed) {
		...
		while(true) {
			...
			if(map[nr][nc] != '0') continue; // 다른 cctv 건너띄기

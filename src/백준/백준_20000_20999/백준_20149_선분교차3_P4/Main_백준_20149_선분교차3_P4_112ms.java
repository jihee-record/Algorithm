...
// 교차여부 판단 후 y = ax + b 로 교점 구하기
public class Main_백준_20149_선분교차3_P4_112ms {
	...
	static void findIntersection(P a, P b, P c, P d) {
		...
		double x = 0.0, y = 0.0;
		if(a.x == b.x) { // AB가 수직 <- CD는 수직 아님
			x = a.x;
			double k_cd = ((double) d.y - c.y) / (d.x - c.x);
			y = k_cd * (x - c.x)  + c.y ;
		}
		
		else if(c.x == d.x) { // CD가 수직 AB는 수직 아님
			x = c.x;
			double k_ab = ((double) b.y - a.y) / (b.x - a.x);
			y = k_ab * (x - a.x) + a.y;
		}
			...
			x = (c.y - k_cd * c.x - a.y + k_ab * a.x) / (k_ab - k_cd); 
			y = k_ab * (x - a.x) + a.y;
		}
